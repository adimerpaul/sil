    ...
    <q-card flat bordered class="q-mb-sm">
      ...
      <q-card-section v-if="header" class="q-pa-sm">
        <div class="row q-col-gutter-sm text-caption">
          ...
          <div class="col-12">
<!--            <pre>{{ (header?.servicios || []).map(s => s.nombre).join(', ') }}</pre>-->
            <!--            <pre>{{canServicios('COAGULOGRAMA (TP,RECUENTO DE PLAQUETAS, APTT)')}}</pre>-->
            <ul>
              <li
                v-for="(s, index) in header?.servicios || []"
                :key="index"
              >
                {{ s.nombre }}
              </li>
